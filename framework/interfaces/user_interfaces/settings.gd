...
func reset() -> void:
	...
	window_mode.enum_items = GameSettings.window_mode_enum
	...
	language.enum_items = GameSettings.language_enum
	language.default_index = GameSettings.supported_language.find(settings.language)
	language.reset()
...
func apply() -> void:
	match current_tab:
		GameSettings.SECTION_WINDOW:
			...
			elif mode in GameSettings.supported_window_mode:
		...
		GameSettings.SECTION_SYSTEM:
			...
			elif lang_idx in range(GameSettings.supported_language.size()):
				settings.language = GameSettings.supported_language[lang_idx]
			else:
				settings.language = TranslationServer.get_locale()
