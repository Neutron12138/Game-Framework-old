...
static func is_string_value(path : String, dict : Dictionary, key : String) -> bool:
	if not dict.has(key):
		push_error("The translation file (\"%s\") is missing key: \"%s\"." % [path, key])
		return false
	if not dict[key] is String:
		push_error("The key \"%s\" of the translation file (\"%s\") must be a String." % [key, path])
		return false
	return true
...
static func is_dictionary_value(path : String, dict : Dictionary, key : String) -> bool:
	...
	if not dict[key] is Dictionary:
		push_error("The key \"%s\" of the translation file (\"%s\") must be a Dictionary." % [key, path])
		return false
	return true
...
static func add_translations(array : Array[Translation]) -> void:
	for i in range(array.size()):
		...
		if is_instance_valid(trans):
			...
			push_error("Invalid translation instance at index: %d." % i)
