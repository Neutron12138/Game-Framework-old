...
func _initialize() -> void:
	...
	ModsManager.initialize_mods(BasicGlobalRegistry.mod_initializers)
...
func _connect_signals() -> void:
	BasicGlobalRegistry.global_events.mod_enabled.connect(ModsManager.on_mod_enabled)
	BasicGlobalRegistry.global_events.mod_disabled.connect(ModsManager.on_mod_disabled)
	BasicGlobalRegistry.global_events.mod_priority_changed.connect(ModsManager.on_mod_priority_changed)
...
func _load_modifications() -> void:
	ModsManager.load_mods_settings()
	var path : String = FilesystemUtilities.get_executable_directory() + ModsManager.MOD_DIRNAME
	ModsManager.load_modifications(path)
	
	if not BasicGlobalRegistry.mods_settings.get_value(
		ModsManager.SECTION_GLOBAL,
		ModsManager.KEY_ENABLE_MODS,
		false):
		return
	
	ModsManager.load_mods_files()
